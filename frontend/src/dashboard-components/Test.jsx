...
function Test() {
    ...
    const { getRootProps: getButtonProps, getInputProps: getButtonInputProps } = useDropzone({ onDrop, noClick: false, noKeyboard: true });
                                ...
                                <ul {...getRootProps(
                                    ...
                                    {files.map((file, index) => (
                                        <li key={index}
                                            ...
                                            <div
                                                className="overflow-hidden text-overflow-ellipsis whitespace-nowrap">{file.name}</div>
                                            <button className="btn btn-ghost" onClick={() => deleteFile(file.path)}>🗑️
                                            </button>
                                        </li>
                                    ))}
                                </ul>
                                <button {...getButtonProps({className: "btn bg-third hover:bg-fourth 1/3 lg:w-1/4 text-white"})}>
                                    <input {...getButtonInputProps()} directory="" webkitdirectory=""/>
                                    Add More Files
                                </button>
                            </div>
                        )}
                    </div>
                <button onClick={HandleSubmit} className="btn w-1/3 text-white bg-first hover:bg-second">Submit</button>
            </div>
        </div>
    );
}
