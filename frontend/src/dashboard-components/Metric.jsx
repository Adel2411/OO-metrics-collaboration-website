...
function Metric() {
  ...
  const displayListElement = (text) => {
    return (
      <div className="py-3 shadow shadow-black flex justify-around rounded-box bg-second">
  ...
  return (
    <div className=" bg-third">
      <div className="overflow-y-auto h-screen w-full fixed flex flex-col items-center gap-10">
        ...
        <ul className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {metrics.map((metric) => (
            <div key={metric.id}>{displayListElement(metric.name)}</div>
          ))}
        </ul>
